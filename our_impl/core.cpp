...
ErrorCode GetNextAvailRes(DocID* p_doc_id, unsigned int* p_num_res, QueryID** p_query_ids)
{
    ...
    *p_num_res=0;
    *p_query_ids=0;
    if(mAvailableDocs.empty()) return EC_NO_AVAIL_RES;
    Document res = mAvailableDocs.front();
    mAvailableDocs.pop();
    *p_doc_id = res.id;
...
void* ThreadFunc(void *param)
{
    ...
    while (1)
    {
        ...
        {
            pthread_mutex_lock(&mPendingDocs_mutex);
            while ( mPhase == PH_IDLE || ( mPhase == PH_01 && (mPendingDocs.empty() || mPendingIndex>= mPendingDocs.size()))){
            ...
            Document doc = mPendingDocs[mPendingIndex++];
            pthread_cond_broadcast(&mPendingDocs_cond);
            pthread_mutex_unlock(&mPendingDocs_mutex);
